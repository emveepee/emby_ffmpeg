...
static int seg_write_packet(AVFormatContext *s, AVPacket *pkt)
{
    ...
    if (seg->avf->streams[st->index]->codecpar->extradata_size == 0 && st->codecpar->extradata_size > 0) {
        ret = ff_alloc_extradata(seg->avf->streams[st->index]->codecpar, st->codecpar->extradata_size);
        if (ret < 0) {
            av_log(s, AV_LOG_WARNING, "Unable to add extradata to output parameters. Output segments may be invalid.\n");
            goto calc_times;
        }
        memcpy(seg->avf->streams[st->index]->codecpar->extradata, st->codecpar->extradata, st->codecpar->extradata_size);
        seg->avf->streams[st->index]->codecpar->extradata_size = st->codecpar->extradata_size;
    }
