...
static int qsv_decode_preinit(AVCodecContext *avctx, QSVContext *q, enum AVPixelFormat pix_fmt, mfxVideoParam *param)
{
    ...
    if (avctx->hw_device_ctx && !avctx->hw_frames_ctx && ret == AV_PIX_FMT_QSV) {
        ...
        hwframes_ctx->initial_pool_size = 32 + avctx->extra_hw_frames;
...
static int find_start_offset(mfxU8 data[1024])
{
...
static int parse_sei_h264(AVCodecContext* avctx, QSVContext* q, AVFrame* out)
{
    ...
    GetBitContext gb = { 0 };
    mfxU8 data[2048] = { 0 };
    mfxPayload payload = { 0, .Data = &data[0], .BufSize = sizeof(data) };
    mfxU64 ts;
    int ret;
...
static int parse_sei_hevc(AVCodecContext* avctx, QSVContext* q, AVFrame* out)
{
    HEVCSEI sei = { 0 };
    HEVCParamSets ps = { 0 };
    GetBitContext gb = { 0 };
    ...
    while (1) {
        ...
        switch (payload.Type) {
            ...
            case SEI_TYPE_CONTENT_LIGHT_LEVEL_INFO:
                continue;
            // Temporarily disabled to avoid flickering
            ////case SEI_TYPE_MASTERING_DISPLAY_COLOUR_VOLUME:
    ...
    if (out)
        return ff_set_side_data(avctx, &sei, NULL, out);
...
static int parse_sei_mpeg12(AVCodecContext* avctx, QSVContext* q, AVFrame* out)
{
    ...
    while (1) {
        ...
        ff_mpeg_decode_user_data(avctx, mpeg_ctx, &data[start], (int)((payload.NumBit + 7) / 8) - start);
                      ...
                      const AVPacket *avpkt)
{
    ...
    if (*sync) {
        ...
        case AV_CODEC_ID_HEVC:
            ret = parse_sei_hevc(avctx, q, out_frame->frame);
            break;
        default:
            ret = 0;
        }
