...
static void show_help_protocol(const char *name)
{
    ...
    p = avio_find_protocol(bp.str);
    namelen = bp.len - 1;

    av_bprint_finalize(&bp, NULL);

    if (!p || namelen < 1 || strncmp(name, p->name, namelen)) {
    ...
    if (p->default_whitelist)
        printf("    Default Whitelist: %s.\n", p->default_whitelist);

    if (p->priv_data_class)
        show_help_children(p->priv_data_class, AV_OPT_FLAG_DECODING_PARAM | AV_OPT_FLAG_ENCODING_PARAM);
}
...
static void show_help_filter(const char *name)
{
    ...
    if (filter_context->nb_outputs == 0) {
        filter_context->outputs= av_mallocz_array(1, sizeof(AVFilterLink*));
    ...
    if (f->formats_state ==  FF_FILTER_FORMATS_QUERY_FUNC) {
        ret = f->formats.query_func(filter_context);
    } else {
        ret = avfilter_query_formats(filter_context);
    }
    ...
    for (i = 0; i < count; i++) {
        ...
        if (filter_context && i < filter_context->nb_inputs)
            print_link_formats(f->inputs[i].type, filter_context->inputs[i]->outcfg.formats);
        printf("\n");
    }
    ...
    for (i = 0; i < count; i++) {
        ...
        if (filter_context && i < filter_context->nb_outputs)
            print_link_formats(f->outputs[i].type, filter_context->outputs[i]->incfg.formats);
        printf("\n");
   }
